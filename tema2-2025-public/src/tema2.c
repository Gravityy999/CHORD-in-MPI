...
void build_finger_table() {
    ...
    for(int i = 0; i < M; i++){
        Finger aux;
        aux.start = (self.id + two_pow) % 16;
        aux.node = find_successor_simple(aux.start);
        self.finger[i] = aux;
        two_pow = two_pow * 2; 
    }
...
int closest_preceding_finger(int key) {
     ...
     ******************************************************/
    for(int i = M - 1; i >= 0; i--){
        int aux = self.finger[i].node;
        if(aux != self.id && in_interval(aux, self.id, key)){
            return aux;
        }
    }
...
void handle_lookup_request(LookupMsg *msg) {
    ...
    msg->path[msg->path_len++] = msg->current_id;
    
    // Cazul special: cheia este exact self.id
    if (msg->key == self.id) {
        MPI_Send(msg, sizeof(LookupMsg), MPI_BYTE, rank_from_id(msg->initiator_id), TAG_LOOKUP_REP, MPI_COMM_WORLD);
    }
    //Cazul normal: cheia este în intervalul (self.id, successor]
    else 
...
int main(int argc, char **argv) {
    ...
    ************************************************************/
    ...
    int done_received = 0;
    int lookups_completed = 0;
    int done_sent = 0;
    MPI_Status status;
    LookupMsg msg;
    //Trimit DONE daca nu am Lookup-uri
    if(nr_lookups == 0){
    ...
    while(1){
        ...
        switch(tag){
            ...
            case TAG_LOOKUP_REP: {
                ...
                if(lookups_completed == nr_lookups){
                    for(int i = 0; i < world_size; i++){
                        if(i != world_rank)
        ...
        if(done_received == world_size - 1 && done_sent){
            break;
        }
    }
    MPI_Finalize();
